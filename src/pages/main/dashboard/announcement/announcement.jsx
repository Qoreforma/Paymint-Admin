    ...
    data?.data?.forEach((item) => {
      if (item?._id === id) {
        setFormData({
          ...
          isPersonalised: item?.isPersonalised || false,
          failedNote: item?.failedNote ? item?.failedNote : "No note",
          channels: item?.channels,
          dateCreated: item?.createdAt,
          dispatchDate: item?.dispatchTime,
          nextBatchAt: item?.nextBatchAt || null,
          batchProgress: item?.batchProgress || null,
        });
      }
    });
    setEditedId(id);
  ...
  return (
    <React.Fragment>
      ...
      <Content>
                          ...
                          <DataTableItem key={item?._id} className="text-secondary">
                            ...
                            <DataTableRow size="md">
                              <span className="tb-lead ccap">
                                {truncateText(`${item?.creatorId?.firstName} ${item?.creatorId?.lastName}`, 5)}{" "}
                              </span>
                            </DataTableRow>

                            <DataTableRow size="md">
                              <span className="text-capitalize">
                                {item?.channels?.join(", ")?.replaceAll("_", " ")}
                              </span>
                            </DataTableRow>
                            <DataTableRow size="md">
        ...
        <Modal isOpen={deleteConfirm} toggle={handleCancelDelete} className="modal-dialog-centered">
          <ModalBody>
            <a
              href="#cancel"
              onClick={(ev) => {
                ...
                <Row className="gy-3">
                  <Col size="12">
                    <ul className="align-center flex-wrap flex-sm-nowrap gx-4 gy-2">
                      <li>
                        <Button onClick={handleConfirmDelete} color="primary" size="md">
                          Proceed
                        </Button>
                      </li>
                      <li>
          ...
          <ModalBody>
            ...
            <div className="nk-tnx-details mt-sm-3">
              <Row className="gy-2">
                ...
                <Col lg={4}>
                  <span className="sub-text">Personalised</span>
                  <Badge
                    className="badge-sm badge-dot has-bg  d-sm-inline-flex"
                    color={formData.isPersonalised ? "success" : "secondary"}
                  >
                    <span className="ccap">{formData.isPersonalised ? "Yes" : "No"}</span>
                  </Badge>
                </Col>
                <Col lg={12}>
                  <span className="sub-text">Body</span>
                  <div className="rich-text" dangerouslySetInnerHTML={{ __html: formData.body }} />
                </Col>
                <Col lg={4}>
                ...
                <Col lg={4}>
                  ...
                  <Badge className="badge-sm badge-dot has-bg d-sm-inline-flex" color={statusColor(formData.status)}>
                  ...
                  <span className="caption-text">
                    {formData?.channels?.map((item, index) => (
                      <span key={index} className="text-capitalize">
                        {item?.replaceAll("_", " ")}
                        {index !== formData?.channels?.length - 1 && ","}{" "}
                      </span>
                    ))}
                  </span>
                </Col>
                ...
                <Col lg={4}>
                  <span className="sub-text">Dispatch date</span>
                  <span className="caption-text">
                    {formData.dispatchDate ? formatDateWithHyphen(formData.dispatchDate) : "Not scheduled"}
                  </span>
                </Col>
                {formData.nextBatchAt && (
                  <Col lg={4}>
                    <span className="sub-text">Next Batch At</span>
                    <span className="caption-text">{formatDateWithHyphen(formData.nextBatchAt)}</span>
                  </Col>
                )}
              </Row>

              {/* Batch Progress Section */}
              {formData.batchProgress && Object.keys(formData.batchProgress).length > 0 && (
                <>
                  <h6 className="mt-3 mb-2">Batch Progress</h6>
                  <Row className="gy-2">
                    {Object.entries(formData.batchProgress).map(([channel, progress]) => (
                      <Col lg={12} key={channel}>
                        <div className="card border p-3">
                          <div className="d-flex justify-content-between align-items-center mb-2">
                            <span className="sub-text text-uppercase fw-bold">{channel}</span>
                            <Badge color={progress.completed ? "success" : "warning"}>
                          ...
                          <Row>
                            <Col size="4">
                              ...
                              <span className="caption-text d-block">{progress.total}</span>
                            </Col>
                            <Col size="4">
                              <span className="sub-text">Sent</span>
                              <span className="caption-text d-block text-success">{progress.sent}</span>
                            </Col>
                            <Col size="4">
                              <span className="sub-text">Failed</span>
                              <span className="caption-text d-block text-danger">{progress.failed}</span>
                            </Col>
                          </Row>
                          <div className="mt-2">
