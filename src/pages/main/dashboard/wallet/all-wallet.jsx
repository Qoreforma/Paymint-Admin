...
const AllWalletTransactionListPage = () => {
  ...
  const { isLoading, data } = useGetWalletTransactions(currentPage, itemsPerPage, status, search, startDate, endDate);
    ...
    <React.Fragment>
      ...
      <Content>
        <BlockHead size="sm">
          <BlockBetween>
            <BlockHeadContent>
              <BlockTitle>All Wallet Transactions</BlockTitle>
            </BlockHeadContent>
          </BlockBetween>
        </BlockHead>

        <WalletTxnTable showStats={true} type={"all"} data={data?.data} isLoading={isLoading} />
      </Content>
    </React.Fragment>
  );
};
