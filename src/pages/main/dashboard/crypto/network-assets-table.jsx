...
import { Card, DropdownItem, DropdownMenu, DropdownToggle, UncontrolledDropdown, Modal, ModalBody } from "reactstrap";
import {
...
import { findUpper, formatDateWithHyphen, formatDateWithTime, formatter, tableNumbers } from "../../../../utils/Utils";
...
const NetworkAssetsTable = ({ data, isLoading, network }) => {
  ...
  const { mutate: deleteAsset } = useDeleteCryptoAssets(editedId, networkId);
  const { mutate: toggleSale } = useToggleSaleActivation(editedId, networkId);
  const { mutate: togglePurchase } = useTogglePurchaseActivation(editedId, networkId);
    ...
    >
      <Block>
        <Block className={"mb-2"}>
          ...
          {selected?.length > 0 && (
            <>
              {hasPermission("crypto_sell.manage") && (
                <Button
          ...
          <div className="card-inner border-bottom">
            <div className="card-title-group">
              <div className="card-title">
                <h5 className="title">{network?.name} Crypto Assets</h5>
              </div>
                        ...
                        <DataTableItem key={item?._id} className="text-secondary">
                          ...
                          <DataTableRow>
                            <div className="user-card">
                              <UserAvatar
                                theme={item?.icon}
                                className="sm d-none d-md-inline"
                          ...
                          <DataTableRow>
                            <div className="custom-control-sm custom-switch">
                              ...
                              <label className="custom-control-label" htmlFor={item?._id}>
                                <span
                                  className={`ccap fw-medium d-none d-md-inline ${item?.saleActivated ? "text-success" : ""}`}
                                >
                          ...
                          <DataTableRow>
                            <div className="custom-control-sm custom-switch">
                              ...
                              <label className="custom-control-label" htmlFor={item?.name}>
                                <span
                                  className={`ccap fw-medium d-none d-md-inline ${item?.purchaseActivated ? "text-success" : ""}`}
                                >
                ...
                {editFormData?.networks?.map((item, index) => (
                  <Row className="gy-1" key={index}>
                    ...
                    <Col lg={6}>
                      ...
                      <span className="caption-text ccap" style={{ wordBreak: "break-all" }}>
                        {item?.address}
                      </span>
                    </Col>
                  </Row>
...
export default NetworkAssetsTable;
