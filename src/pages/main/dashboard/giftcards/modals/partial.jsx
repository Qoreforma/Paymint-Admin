...
import { useSecondApproveGiftcard } from "../../../../../api/giftcard";
import { generateSignature, useUploadImages } from "../../../../../api/uploadimage";
import { Button, Col, Icon } from "../../../../../components/Component";

const PartialApprovalModal = ({ modal, closeModal, editedId, giftcardId, single = true }) => {
  ...
  const { mutate: secondApprove } = useSecondApproveGiftcard(editedId, giftcardId, single);
  ...
  const submitForm = (e) => {
    secondApprove({
      reviewedAmount: e.review_rate,
      reviewNote: e.review_note,
      ...(uploadedPreview && { reviewProof: uploadedPreview }),
    });
  ...
  const uploadImageToImageKit = async (type) => {
    bulkImageUpload(previewImage[0]);
  };
    ...
    <>
      <Modal isOpen={modal} toggle={close} className="modal-dialog-centered" size="md">
        <ModalBody>
          ...
          <div className="p-2">
            <h5 className="title">Partial Approval Request</h5>
            <div className="mt-4">
