...
const ProvidersServices = () => {
  ...
  const { isLoading, data: services } = useGetProviderServices(providerId);

  const { data: provider } = useGetProviderInfo(providerId);
  const { mutate: toggleService } = useUpdateProviderService(providerId);
                      ...
                      {services?.data?.map((item, idx) => {
                        return (
                          <DataTableItem key={idx} className="text-secondary">
                            ...
                            <DataTableRow>
                              <span className="ccap">{item.type.replaceAll("_", " ")}</span>
                            </DataTableRow>
                            <DataTableRow>
                              <div className="custom-control-sm custom-switch">
                                <input
                                  ...
                                  name={item.name}
                                  onClick={() => {
                                    setEditedId(item.serviceId);
                                    toggleService({
                                      code: item.serviceId,
                                      type: item.productType,
                                      body: { isActive: item?.status === "active" ? false : true },
                                    });
                                  }}
                                  id={item.serviceId}
                                />
                                <label className="custom-control-label" htmlFor={item.serviceId}>
                                  <span
                                    className={`ccap fw-medium d-none d-md-inline ${item?.status === "active" ? "text-success" : ""}`}
                                  >
                                    {item.status}
                                  </span>
                                </label>
                            ...
                            {hasPermission("system.manage_products") && (
                              <DataTableRow className="tb-odr-action">
                                <div className="tb-odr-btns d-md-inline">
                                  <Button
                                    ...
                                    onClick={(ev) => {
                                      navigate(
                                        `/service-products/${providerId}/${item.serviceId}/${item.productType}`,
                                        {
                                          state: { providerName: item?.name },
                                        },
                                      );
                                    }}
                                  >
