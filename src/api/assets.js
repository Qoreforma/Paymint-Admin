...
export const useGetAssetsTransactions = (
  ...
  return useQuery(
    ["getAssets", page, limit, status, search, channel, startDate, endDate, user, cryptoId, tradeType],
    async () => {
      const request = await instance
        .get(
          BACKEND_URLS.crypto +
            `?page=${page}&limit=${limit}${statusTerm}${searchTerm}${channelTerm}${userTerm}${tradeTypeTerm}${startDateTerm}${endDateTerm}${cryptoIdTerm}`,
        )
        .then((res) => res?.data)
        .catch((err) => {
          throw err;
