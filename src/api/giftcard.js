...
export const useGetGiftcardTransactions = (
  page,
  limit,
  status,
  search,
  ...
  return useQuery(
    ["getGiftcard", page, limit, status, search, startDate, endDate, user, tradeType, channel],
    async () => {
      const request = await instance
        .get(
          BACKEND_URLS.giftcard +
            `?page=${page}&limit=${limit}${statusTerm}${searchTerm}${userTerm}${tradeTypeTerm}${channelTerm}${startDateTerm}${endDateTerm}`,
        )
        .then((res) => res?.data)
        .catch((err) => {
          throw err;
